...
void setup() {
  ...
  velocidad_v = new PVector(0, 0.24);
...
void Caida_Fase1() {
  ...
  tiempo_t = str(cronometro/0.1);
  pos = str(((300*posicion.y)/720));
  System.out.println("La velocidad en el instante x= " + ((300*posicion.y)/720) + "m con t = " + cronometro/0.1 + " es de " + velocidad_n + " m/s");
  ...
  velocidad_instante[contador_ingreos_vector_fase1] = velocidad_v.y*0.24; 
  posiciones_antiguas_y[contador_ingreos_vector_fase1] = posicion.y*300/720;
  if (posicion.y>= cuerda || posicion.y*300/720 > altura_puente) { // Se verifica si la persona ya llego a la posicion que indico como cuerda para comenzar la fase 2
    if (posicion.y*300/720 > altura_puente) { // Se verifica si la persona no se choca contra el piso debido a que su posicion es mayor que el limite de la pantalla
      ...
      cronometro_fase2 = cronometro/0.1;
    }
  }
  ...
  if (((300*posicion.y)/720) <= cuerda) {
    ...
    tiempo_graf[contador_ingreos_vector_fase1] = cronometro/0.1;
...
void Caida_Fase2() {
  ...
  beta = redondearDecimales(beta)-0.01;
  Calcular_fuerza("neta fase 2"); // Se calcula la velocidad en cada instante llamando a la subrutina Calcular_fuerza
  if (posicion.y*300/720 >= altura_puente) { // Se verifica si la persona no se ha golpeado contra el fondo de la pantalla
  ...
  strokeWeight(1);
  System.out.println("La velocidad en el instante x= " + (posicion_2 +(cuerda*300/720)) + "m con t = " + cronometro_fase2 + "s es de " + velocidad_2v/0.24 + " m/s");
  textFont(letra2);
  textSize(16);
  fill(#f7b016);
  // Se transforma los valores numeros convertidos a metros para ser impresos
  pos = str((posicion_2 +(cuerda*300/720)));
  velocidad = str(velocidad_2v/0.24);
  ...
  velocidad_instante_2[contador_ingresos_vector_fase2] = velocidad_2v/0.24;
  ...
  cronometro_fase2 = cronometro_fase2 + 0.1;
...
void CreacionEcuaciones_Fase2(float tiempo, float velocidad) {
  ...
  beta = redondearDecimales(beta)-0.01; // Para tener el valor completamente redondeado, se le resta 0.01, debido a un error de ese margen 
  a = pow(2.71, alfa*tiempo)*sin(beta*tiempo);
  b = pow(2.71, alfa*tiempo)*cos(beta*tiempo);
  c = beta * pow(2.71, alfa*tiempo) * cos(beta*tiempo) - alfa*pow(2.71, alfa*tiempo)*sin(beta*tiempo); // Primera parte de la derivada
  d = alfa *pow(2.71, alfa*tiempo) * cos(beta*tiempo) - beta * pow(2.71, alfa*tiempo)*sin(beta*tiempo); // Segunda parte de la derivada
...
void Calcular_fuerza(String fuerza) {
  fuerza = fuerza.toLowerCase();
  if (fuerza.equals("neta fase 1")) {
    velocidad_n= (masa/coeficiente_r)*C1_f1*((-coeficiente_r/masa)*pow(2.71, -coeficiente_r/masa*(cronometro/0.1)))+masa*gravedad/coeficiente_r;
    velo_vector = velocidad_n * 0.24;
  } else if (fuerza.equals("neta fase 2")) {
    p1 =  c1 * pow(2.71, alfa*cronometro_fase2)*sin(beta*cronometro_fase2);
    p2 =  c2 * pow(2.71, alfa*cronometro_fase2)*cos(beta*cronometro_fase2);
    p3 = c1 * (beta*pow(2.71, alfa*cronometro_fase2) * cos(beta*cronometro_fase2) - alfa * pow(2.71, alfa*cronometro_fase2)*sin(beta*cronometro_fase2)); 
    p4 = c2 * (alfa * pow(2.71, alfa*cronometro_fase2) * cos(beta*cronometro_fase2) - beta * pow(2.71, alfa*cronometro_fase2)*sin(beta*cronometro_fase2));
    posicion_2 = p1 + p2 + (masa*gravedad/constante_k); 
    velocidad_2 = p3 + p4;
    posicion_2v = posicion_2*720/300;
    velocidad_2v = velocidad_2 * 0.24;
  }
}
 ...
 */
float redondearDecimales(float valorInicial) {
...
void Crear_Graficas(String fase, String ejex, String ejey1, String ejey2, float[] tiempo_fase, float[] posicion_fase, float[] velocidad_fase, int contador) {
  ...
  if (fase.equals("fase 1")) {
    ...
    for (int c = 1; c <= 12; c++) {
      strokeWeight(2);
      line(-3, -c*12.5, 3, -c*12.5);
      textFont(letra2);
      textSize(11);
      text(c*15 + "m", -25, -c*12.5);
      strokeWeight(3);
    }
    for (int c = 1; c <= 24; c++) {
      strokeWeight(2);
    ...
    for (int c = 1; c <= 9; c++) {
      strokeWeight(2);
      line(-3, -c*12.22, 3, -c*12.22);
      textFont(letra2);
      textSize(11);
      text(c*6 + "m/s", -25, -c*12.22);
      strokeWeight(3);
    }
    for (int c = 1; c <= 24; c++) {
      strokeWeight(2);
 ...
 */
void Limpiar_Arreglos(float velocidad_fase1[], float tiempo_fase1[], float posicion_fase1[], float velocidad_fase2 [], float tiempo_fase2 [], float posicion_fase2[]) {
 ...
 */
void LLenarTabla(float[] tiempo, float[] velocidad, float[] posicion, Table tabla, int contador_saltos, String fase, int contador_fases) {
 ...
 */
void seleccion_menu(int seleccion) {
...
void mouseClicked() {
  ...
  if ((mouseX >= 693 && mouseX <= 748) && (mouseY >= 535 && mouseY <= 584)) {
    if (altura_puente <= 70) {
      ...
      puente_a = aumento*2.40 + puente_a;
      altura_persona = aumento*2.40 + altura_persona;
    }
  }
  if ((mouseX >= 883 && mouseX <= 940) && (mouseY >= 535 && mouseY <= 584)) {
    if (altura_puente >= 240) {
      ...
      altura_puente += aumento;
      // Mueve a la persona con respecto al puente
      puente_a = 2.40*-aumento + puente_a;
      altura_persona = 2.40*-aumento + altura_persona;
    }
  }
